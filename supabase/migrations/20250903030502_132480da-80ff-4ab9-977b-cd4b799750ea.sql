...
CREATE TABLE public.thrift_groups (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT valid_participants CHECK (max_participants > 1 AND max_participants <= 50),
  CONSTRAINT valid_amount CHECK (contribution_amount > 0)
);
...
CREATE TABLE public.transactions (
  ...
  description TEXT,
  reference_id UUID, -- Can reference group_id or contribution_id
...
CREATE INDEX idx_verification_user_id ON public.verification(user_id);
CREATE INDEX idx_verification_status ON public.verification(verification_status);
CREATE INDEX idx_wallets_user_id ON public.wallets(user_id);
CREATE INDEX idx_thrift_groups_status ON public.thrift_groups(status);
CREATE INDEX idx_thrift_groups_creator ON public.thrift_groups(creator_id);
